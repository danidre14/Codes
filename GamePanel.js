const GamePanel = function (gameAreaId, options) {
    /*options can have
        dimensions: width [num], height [num];renderMethod;index; name; showLogs' renderOn: mouse [t/f]; safeRender; startEmpty
    */
    ...
    function resizeCanvas(dontRender) {
        if(!isFullScreen()) {
            ...
            if(vp.w < WIDTH) {
                const newW = vp.w - PAD;
                const newHR = defaultCW / defaultCH;
                gameCanvasDiv.style.width = (newW) + 'px';
                gameCanvasDiv.style.height = (newW / newHR) + 'px';
            } else {
                gameCanvasDiv.style.width = defaultCW + "px";
                gameCanvasDiv.style.height = defaultCH + "px";
            }
            ...
            if(LOCKSIZE) {
                const vp = getViewportSize();
                const canvRatio = defaultCW / defaultCH; //20/100
                const viewRatio = vp.w / vp.h; //10/100 --this is less, so resize by canvas width | 30/100 --this is more, so resize by canvas height
                if(viewRatio < canvRatio) { //width bigger
                ...
                } else if(viewRatio > canvRatio) { //height bigger
    ...
    function setMousePosition(event) {
        var rect = gameCanvas.getBoundingClientRect(), // abs. size of element
            scaleX = gameCanvas.width / rect.width,    // relationship bitmap vs. element for X
            scaleY = gameCanvas.height / rect.height;  // relationship bitmap vs. element for Y

        mouseX = parseInt((event.clientX - rect.left) * scaleX);   // scale mouse coordinates after they have
        mouseY = parseInt((event.clientY - rect.top) * scaleY);     // been adjusted to be relative to element
    ...
    const gameArea = document.getElementById(gameAreaId); //get game area
    const gamePanel = document.createElement("div"); //create game panel
    gamePanel.className = "gamePanel";
    gameArea.appendChild(gamePanel); //put game panel inside game area
    ...
    const gamePanelHTML = `
        ...
        <div class="gamePanelButtonsDiv">
            <button class="gamePanelButton" id="fsBtn">Full Screen</button>
            <button class="gamePanelButton" id="shBtn">Share</button>
        </div>
    `;
    ...
    WIDTH = getWidth();
    HEIGHT = getHeight();
    PAD = (parseInt(window.getComputedStyle(gamePanel, null).getPropertyValue('padding-left')) +
    ...
    function openFullscreen() {
        ...
        } else if (gameCanvasDiv.mozRequestFullScreen) { /* Firefox */
            gameCanvasDiv.mozRequestFullScreen();
        } else if (gameCanvasDiv.webkitRequestFullscreen) { /* Chrome, Safari and Opera */
            gameCanvasDiv.webkitRequestFullscreen();
        } else if (gameCanvasDiv.msRequestFullscreen) { /* IE/Edge */
    ...
    gameCanvasDiv.addEventListener('fullscreenchange', fsChangeHandler, false);
    gameCanvasDiv.addEventListener('mozfullscreenchange', fsChangeHandler, false);
    gameCanvasDiv.addEventListener('MSFullscreenChange', fsChangeHandler, false);
    gameCanvasDiv.addEventListener('webkitfullscreenchange', fsChangeHandler, false);
